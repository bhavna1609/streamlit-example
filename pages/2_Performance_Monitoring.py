...
df = conn.query('SELECT SUM(CREDITS_USED) CREDITS_USED,TO_DATE(START_TIME) START_DATE,WAREHOUSE_NAME FROM  SNOWFLAKE.ACCOUNT_USAGE.WAREHOUSE_METERING_HISTORY  GROUP BY  START_DATE,WAREHOUSE_NAME;')
df

st.line_chart(
    df,
    x = 'START_DATE',
    y = ['CREDITS_USED','WAREHOUSE_NAME'],
