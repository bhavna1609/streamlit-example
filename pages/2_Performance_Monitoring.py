...
df = conn.query('SELECT SUM(CREDITS_USED) CREDITS_USED,TO_DATE(START_TIME) START_DATE,WAREHOUSE_NAME FROM  SNOWFLAKE.ACCOUNT_USAGE.WAREHOUSE_METERING_HISTORY WHERE WAREHOUSE_NAME NOT IN (\'CLOUD_SERVICES_ONLY\') GROUP BY  START_DATE,WAREHOUSE_NAME;', ttl=600)
df
